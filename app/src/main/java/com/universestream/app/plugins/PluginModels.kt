...
@Serializable
data class UniverseStreamPluginManifest(
    ...
    val supportsConfigurationActivity: Boolean
        get() = hasCapability(UniverseStreamPluginContract.CAPABILITY_CONFIGURATION_ACTIVITY) &&
            !configurationActivityAction.isNullOrBlank()

    val usesActivityConfiguration: Boolean
        get() = configurationMode == UniverseStreamPluginContract.CONFIGURATION_MODE_ACTIVITY

    val supportsHostRenderedConfiguration: Boolean
        get() = configurationMode == UniverseStreamPluginContract.CONFIGURATION_MODE_HOST_SCHEMA ||
            (configurationMode != UniverseStreamPluginContract.CONFIGURATION_MODE_ACTIVITY &&
                hasCapability(UniverseStreamPluginContract.CAPABILITY_CONFIGURATION_SCHEMA))

    val canConfigure: Boolean
        get() = supportsHostRenderedConfiguration || supportsConfigurationActivity
}

data class InstalledUniverseStreamPlugin(
    val packageName: String,
    val serviceClassName: String,
    val appLabel: String,
    val manifest: UniverseStreamPluginManifest,
    val enabled: Boolean,
...
data class PluginConfigurationSnapshot(
    val plugin: InstalledUniverseStreamPlugin,
