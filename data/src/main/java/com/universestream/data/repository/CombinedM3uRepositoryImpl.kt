...
import com.universestream.data.local.dao.CombinedM3uProfileDao
import com.universestream.data.local.dao.CombinedM3uProfileMemberDao
import com.universestream.data.local.dao.ProviderDao
import com.universestream.data.local.entity.CombinedM3uProfileEntity
import com.universestream.data.local.entity.CombinedM3uProfileMemberEntity
...
import com.universestream.domain.model.CombinedCategoryBinding
            ...
            if (memberProviders.isEmpty()) {
                ...
                combine(categoryFlows) { arrays ->
                    arrays.toList()
                        .map { it as Triple<*, *, *> }
                        .flatMap { triple ->
                            val member = triple.first as com.universestream.domain.model.CombinedM3uProfileMember
                            val categories = triple.second as List<Category>
                            val hiddenCategoryIds = triple.third as Set<Long>
                            categories
                                .filter {
                                    !it.isVirtual &&
                                        it.id != com.universestream.domain.repository.ChannelRepository.ALL_CHANNELS_ID &&
