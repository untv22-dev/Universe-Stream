...
import com.universestream.data.local.dao.RecordingScheduleDao
import com.universestream.data.local.dao.VirtualGroupDao
import com.universestream.data.local.entity.ProviderEntity
import com.universestream.data.local.entity.RecordingScheduleEntity
import com.universestream.data.mapper.toDomain
import com.universestream.data.mapper.toEntity
import com.universestream.data.preferences.ParentalPinBackupData
import com.universestream.data.preferences.PreferencesRepository
import com.universestream.data.security.CredentialCrypto
import com.universestream.domain.manager.BackupData
...
import com.universestream.domain.manager.RecordingScheduleImportDisposition
import com.universestream.domain.manager.RecordingScheduleImportOutcome
import com.universestream.domain.manager.RecordingScheduleImportSummary
import com.universestream.domain.manager.ProtectedCategoryBackup
import com.universestream.domain.manager.RecordingManager
import com.universestream.domain.manager.ScheduledRecordingBackup
...
class BackupManagerImpl @Inject constructor(
    ...
    override suspend fun exportConfig(uriString: String): com.universestream.domain.model.Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            } ?: return@withContext com.universestream.domain.model.Result.error("Failed to open output stream")

            com.universestream.domain.model.Result.success(Unit)
        } catch (e: Exception) {
            com.universestream.domain.model.Result.error("Failed to export backup: ${e.message}", e)
        }
    }
    ...
    ): com.universestream.domain.model.Result<BackupImportResult> = withContext(Dispatchers.IO) {
        try {
            val backupData = readBackupData(uriString)
                ?: return@withContext com.universestream.domain.model.Result.error("Failed to open input stream")

            if (backupData.version > CURRENT_BACKUP_VERSION) {
                return@withContext com.universestream.domain.model.Result.error("Unsupported backup version")
            }
            if (backupData.isStructurallyEmpty()) {
                return@withContext com.universestream.domain.model.Result.error("Backup file does not contain any importable data")
            }
            if (!verifyChecksum(backupData)) {
                return@withContext com.universestream.domain.model.Result.error("Backup file is corrupted (checksum mismatch)")
            }
            ...
            com.universestream.domain.model.Result.success(
                BackupImportResult(
            ...
            com.universestream.domain.model.Result.error("Failed to import backup: ${e.message}", e)
        }
    }
    ...
    private suspend fun restorePreferences(prefs: Map<String, String>) {
        ...
        prefs["appLandingDestination"]?.takeIf { it.isNotBlank() }?.let { savedDestination ->
            preferencesRepository.setAppLandingDestination(
                com.universestream.domain.model.AppLandingDestination.fromStorage(savedDestination)
            )
        }
        ...
        val legacyDecoderMode = prefs["playerDecoderMode"]
            ...
            ?.let { savedMode ->
                com.universestream.domain.model.DecoderMode.entries
        ...
        prefs["playerAudioOutputPreference"]?.takeIf { it.isNotBlank() }?.let { savedPreference ->
            val preference = com.universestream.domain.model.AudioOutputPreference.entries
        ...
        prefs["playerSurfaceMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val surfaceMode = com.universestream.domain.model.PlayerSurfaceMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (surfaceMode != null) {
        ...
        prefs["playerLiveStreamFormatMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val formatMode = com.universestream.domain.model.LiveStreamFormatMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (formatMode != null) {
        ...
        (prefs["playerVodHttpProtocolMode"] ?: prefs["playerMovieHttpProtocolMode"])?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val protocolMode = com.universestream.domain.model.VodHttpProtocolMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
    ...
    private suspend fun restoreSavedLibrary(
        ...
        val categoriesByProviderId = mutableMapOf<Long, List<com.universestream.domain.model.Category>>()
    ...
    private fun resolveProviderIdMap(
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.universestream.domain.model.Provider>
    ...
    private suspend fun restorePlaybackHistory(
        history: List<com.universestream.domain.model.PlaybackHistory>,
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.universestream.domain.model.Provider>,
        conflictStrategy: BackupConflictStrategy
    ) {
...
internal fun com.universestream.domain.model.RecordingItem.toScheduledRecordingBackup(
    provider: com.universestream.domain.model.Provider,
...
internal suspend fun importScheduledRecordingBackups(
    ...
    storedProviders: List<ProviderEntity>,
    existingSchedules: MutableList<com.universestream.domain.model.RecordingItem>,
    conflictStrategy: BackupConflictStrategy,
    recordingManager: RecordingManager,
...
private fun com.universestream.domain.model.Provider.toSecureEntityForBackup(
...
private val PROVIDER_LIST_TYPE: Type = object : TypeToken<List<com.universestream.domain.model.Provider>>() {}.type
private val FAVORITE_LIST_TYPE: Type = object : TypeToken<List<com.universestream.domain.model.Favorite>>() {}.type
private val VIRTUAL_GROUP_LIST_TYPE: Type = object : TypeToken<List<com.universestream.domain.model.VirtualGroup>>() {}.type
private val PLAYBACK_HISTORY_LIST_TYPE: Type = object : TypeToken<List<com.universestream.domain.model.PlaybackHistory>>() {}.type
