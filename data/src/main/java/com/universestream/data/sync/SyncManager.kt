...
import com.universestream.data.local.dao.CatalogSyncDao
import com.universestream.data.local.dao.CategoryDao
import com.universestream.data.local.dao.ChannelDao
import com.universestream.data.local.dao.EpisodeDao
...
import com.universestream.data.local.dao.SeriesCategoryHydrationDao
import com.universestream.data.local.dao.SeriesDao
import com.universestream.data.local.dao.TmdbIdentityDao
...
import com.universestream.data.local.entity.ChannelEntity
import com.universestream.data.local.entity.ChannelGuideSyncEntity
...
import com.universestream.data.parser.M3uParser
...
import com.universestream.data.remote.stalker.StalkerPlaybackMode
import com.universestream.data.remote.stalker.StalkerProvider
...
import com.universestream.data.remote.stalker.StalkerTrafficCoordinator
import com.universestream.data.remote.dto.XtreamCategory
import com.universestream.data.remote.dto.XtreamSeriesItem
...
import com.universestream.data.util.AdultContentClassifier
...
class SyncManager @Inject constructor(
    ...
    private val preferencesRepository: com.universestream.data.preferences.PreferencesRepository,
    ...
    suspend fun syncEpg(
        ...
        val providerEntity = providerDao.getById(providerId)
            ?: return com.universestream.domain.model.Result.error("Provider $providerId not found")
        ...
        return if (providerEntity.type == ProviderType.STALKER_PORTAL) {
            ...
            withProviderLock(providerId) {
                val freshProviderEntity = providerDao.getById(providerId)
                    ?: return@withProviderLock com.universestream.domain.model.Result.error("Provider $providerId not found")
    ...
    private suspend fun syncEpgLocked(
        providerEntity: com.universestream.data.local.entity.ProviderEntity,
        force: Boolean,
    ...
    suspend fun rebuildXtreamIndex(
        ...
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.universestream.domain.model.Result.error("Index rebuild is only available for Xtream providers")
        }
    ...
    suspend fun processQueuedXtreamIndexJobs(
        ...
        val providerEntity = providerDao.getById(providerId)
            ?: return@lock com.universestream.domain.model.Result.error("Provider $providerId not found")
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.universestream.domain.model.Result.success(Unit)
        }
    ...
    private suspend fun processStalkerSummaryIndexSection(
        ...
        data class StalkerCategoryFetchResult(
            ...
            val result: Result<com.universestream.data.remote.stalker.StalkerPagedResult<out Any>>
        )
            ...
            when (val result = fetchStalkerWildcardSummaryPageWithRecovery(api, contentType, wildcardCategory.categoryId, nextPage)) {
                is Result.Success -> {
                    ...
                    val anomaly = detectStalkerPageAnomaly(
                        hydration = currentHydration,
                        requestedPage = nextPage,
                        pagedResult = com.universestream.data.remote.stalker.StalkerPagedResult(
                            items = visibleItems,
    ...
    private suspend fun markStalkerAttemptStarted(
        ...
        when (contentType) {
            ContentType.MOVIE -> movieCategoryHydrationDao.upsert(
                com.universestream.data.local.entity.MovieCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
            ...
            ContentType.SERIES -> seriesCategoryHydrationDao.upsert(
                com.universestream.data.local.entity.SeriesCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
                    lastHydratedAt = hydration?.lastHydratedAt ?: 0L,
                    itemCount = hydration?.itemCount ?: 0,
    ...
    private suspend fun fetchStalkerSummaryPageWithRecovery(
        ...
        categoryId: Long,
        page: Int
    ): Result<com.universestream.data.remote.stalker.StalkerPagedResult<out Any>> {
    ...
    private suspend fun fetchStalkerSummaryFirstPageFallback(
        ...
        contentType: ContentType,
        categoryId: Long
    ): Result<com.universestream.data.remote.stalker.StalkerPagedResult<out Any>> = when (contentType) {
        ContentType.MOVIE -> when (val result = api.getVodStreams(categoryId)) {
            is Result.Success -> Result.success(
                com.universestream.data.remote.stalker.StalkerPagedResult(
                    items = result.data,
                    page = 1,
                    totalPages = 1,
    ...
    private fun detectStalkerPageAnomaly(
        hydration: StalkerHydrationSnapshot?,
        requestedPage: Int,
        pagedResult: com.universestream.data.remote.stalker.StalkerPagedResult<out Any>,
        pageFingerprint: String?
    ): String? {
    ...
    private suspend fun syncStalker(
        ...
        if (force || ContentCachePolicy.shouldRefresh(metadata.lastMovieSuccess, ContentCachePolicy.CATALOG_TTL_MILLIS, now)) {
            ...
            val categories = when (val categoriesResult = api.getVodCategories()) {
                is com.universestream.domain.model.Result.Success -> categoriesResult.data
                ...
                is com.universestream.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
            }
            emitCatalogSyncProgress(
                section = Section.VOD,
    ...
    private suspend fun syncJellyfin(
        ...
        try {
            ...
            if (resolvedMovies is com.universestream.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Movies error: ${resolvedMovies.message}")
            if (resolvedSeries is com.universestream.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Series error: ${resolvedSeries.message}")

            if (resolvedMovies is com.universestream.domain.model.Result.Error && resolvedSeries is com.universestream.domain.model.Result.Error) {
            ...
            val movieEntities = (resolvedMovies as? com.universestream.domain.model.Result.Success)?.data.orEmpty()
            val seriesEntities = (resolvedSeries as? com.universestream.domain.model.Result.Success)?.data.orEmpty()
            ...
            if (movieEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.universestream.domain.model.ContentType.MOVIE.name, listOf(
                        com.universestream.data.local.entity.CategoryEntity(
            ...
            if (seriesEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.universestream.domain.model.ContentType.SERIES.name, listOf(
                        com.universestream.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 2L, name = "Series",
                            type = com.universestream.domain.model.ContentType.SERIES
                        )
                    ))
                }
            }
    ...
    private suspend fun syncStalkerPortalEpg(
        ...
        val insertBuffer = ArrayList<com.universestream.data.local.entity.ProgramEntity>(STALKER_GUIDE_PROGRAM_BATCH_SIZE)
        ...
        fallbackGuideRequests.forEachIndexed { index, request ->
            ...
            runCatching {
                ...
                if (streamResult is com.universestream.domain.model.Result.Error) {
    ...
    private suspend fun loadStalkerChannelsByCategory(
        api: StalkerProvider,
        categories: List<com.universestream.domain.model.Category>,
        onProgress: ((String) -> Unit)?
    ...
    private suspend fun stalkerCatalogAccessDiagnostic(
        ...
        val profile = when (val profileResult = api.getAccountProfile()) {
            is com.universestream.domain.model.Result.Success -> profileResult.data
            else -> return null
        }
    ...
    private fun <T> requireResult(result: com.universestream.domain.model.Result<T>, fallbackMessage: String): T {
        return when (result) {
            is com.universestream.domain.model.Result.Success -> result.data
            is com.universestream.domain.model.Result.Error -> throw IllegalStateException(result.message.ifBlank { fallbackMessage }, result.exception)
            is com.universestream.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
        }
    }
