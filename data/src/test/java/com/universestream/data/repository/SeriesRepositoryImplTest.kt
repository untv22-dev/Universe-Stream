...
import com.universestream.data.local.entity.EpisodeBrowseEntity
import com.universestream.data.local.entity.EpisodeEntity
import com.universestream.data.local.entity.SeriesEntity
import com.universestream.data.local.entity.SeriesBrowseEntity
...
import com.universestream.data.remote.dto.XtreamSeason
import com.universestream.data.remote.dto.XtreamSeriesInfoResponse
import com.universestream.data.remote.stalker.StalkerCategoryRecord
import com.universestream.data.remote.stalker.StalkerEpisodeRecord
...
import com.universestream.data.remote.stalker.StalkerSeasonRecord
import com.universestream.data.remote.stalker.StalkerSeriesDetails
...
class SeriesRepositoryImplTest {
    ...
    fun `stalker series preview loads only first page`() = runTest {
        ...
        whenever(categoryDao.getByProviderAndType(7L, ContentType.SERIES.name)).thenReturn(
            flowOf(listOf(com.universestream.data.local.entity.CategoryEntity(providerId = 7L, categoryId = 77L, name = "Drama", type = ContentType.SERIES)))
        )
        whenever(providerDao.getById(7L)).thenReturn(
            ProviderEntity(
                id = 7L,
    ...
    fun `getSeriesDetails falls back to remote series id lookup`() = runTest {
        ...
        val series = (result as com.universestream.domain.model.Result.Success).data
