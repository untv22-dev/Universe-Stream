...
FRAMES=$(mktemp -d)
python3 music.py "$FRAMES/music_raw.wav" 15
ffmpeg -loglevel error -y -i "$FRAMES/music_raw.wav" -af loudnorm=I=-18:TP=-1.5:LRA=11 -ar 48000 "$FRAMES/music.wav"
NODE_PATH="${NODE_PATH:-$(npm root -g)}" node render.js "$FRAMES"
mkdir -p out
ffmpeg -loglevel error -y -framerate 30 -i "$FRAMES/f%04d.png" -i "$FRAMES/music.wav" \
  -map 0:v -map 1:a -c:v libx264 -preset slow -crf 16 -profile:v high -pix_fmt yuv420p -r 30 \
  -c:a aac -b:a 192k -ar 48000 -ac 2 -t 15 -movflags +faststart out/UniverseIPTV_Ad_15s_1080x1920_master.mp4
